...
from live_suppport.models import Chat, ChatMessage, SupportGroup
from live_suppport.forms import ChatMessageForm, ChatForm


@permission_required('live_suppport.chat_admin')
def join_chat(request, chat_id):
    ...
    if request.user.is_authenticated :
        ...
        message.chat = chat  # Assign the chat field
        message.save()
...
def end_chat(request, chat_id):
    ...
    if request.POST.get('end_chat') == 'true':
        ...
        message.chat = chat  # Associate the message with the chat
...
def get_messages(request):
    ...
    pending_chats_list = [{
        ...
        'url': reverse('live_suppport.views.join_chat', args=[chat.id]),
...
def client_chat(request, chat_uuid):
    ...
    return render_to_response('live_suppport/live_support.html', params,
                              context_instance=RequestContext(request))

# def admin_active(user):
...
def start_chat(request, support_group_id=None):
    ...
    return render(request, 'live_suppport/start_chat.html', params)
